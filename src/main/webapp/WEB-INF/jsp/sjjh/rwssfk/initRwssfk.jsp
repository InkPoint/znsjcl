        ...
        <div class="col-xs-12 col-md-12">
            <div class="widget">
                ...
                <div class="widget-body">
                    ...
                    <!--添加任务模态框-->
                    <div class="modal fade" id="rwssxjModal" role="dialog" aria-label="myModalLabel">
                                ...
                                <div class="modal-body">
                                    <form id="editForm" method="post" class="form-horizontal">
                                        <div class="form-group">
                                            <label class="col-sm-2 control-label">表中文名称:</label>

                                            <div class="col-sm-3">
                                                <select id="bzwmc" name="bzwmc" class="js-data2-example-ajax"
                                                        style="width: 150px;">
                                                    <option></option>
                                                </select>
                                            </div>

                                            <label class="col-sm-2 control-label">表数据量：</label>

                                            <div class="col-sm-3">
                                                <input id="bsjl" name="bsjl" type="text"
                                                       onkeyup="value=value.replace(/[^\d]/g,'')" style="width:100px;"
                                                       class="form-control">
                                                <input type="hidden" id="hiddenrwid" name="hiddenrwid">
                                            </div>
...
//        //成功方法
        var myfunctions = function () {
            //刷新当前页面
            $('#rwssxj_table').bootstrapTable('refresh');
            $(".js-data2-example-ajax").val("").select2();
            $("#bsjl").val("");
            //$("#bsjl").empty();
        };
        var showModel = function () {
            //添加成功后显示模态框
            $("#rwssxjModal").model('show');
        };
    ...
    //Modal验证销毁重构
    $("#rwssxjModal").on('hidden.bs.modal', function () {
    ...
    function inittabs(rwmcs) {
        ...
        $('#taskList_table').bootstrapTable({
            url: "${pageContext.request.contextPath}/sjjh/rwssfk/list",
...
//                },
                {
                    ...
                    title: '任务实施细节',
                    formatter: function (value, row, index) {
                        return '<a style="color:#057afd;" onclick="rwsssj(\'' + row.rwid + '\')">查看</a>';
                    }
                }, {
                    field: 'wcjd',
                    align: 'center',
                    ...
                    formatter: function (value, row, index) {
                        ...
                        if (value == undefined) {
                            return ["<div onclick='wcjdbj(" + value + " ," + row.rwid + ")'><div>0%</div></div>"];
    ...
    //任务实施细节
    function rwsssj(rwid) {
        $('#rwssxjModal').modal('show');
        rwid1 = rwid;
        document.getElementById("hiddenrwid").value=rwid;
        insert_rwssxj_tb(rwid);
            ...
            fields: {
                bzwmc: {
                    ...
                    validators: {
                        ...
                        remote: {
                            url: "${pageContext.request.contextPath}/sjjh/rwssfk/nameValidation",
                            message: '表数据已添加',
//                            delay: 1000,//延迟
                            type: 'POST',
                            data:function(validator){
                                return{
                                    rwid:$('[name="hiddenrwid"]').val()
                                }
                            }
                        }
                    }
