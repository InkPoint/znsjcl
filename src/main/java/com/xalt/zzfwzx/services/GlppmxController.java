...
public class GlppmxController extends WebServiceSupport {
    ...
    //初始化匹配模型界面
    @NoLoginValidate
    public void initGlppmx() {
        DelegateMapper mapper = getMapper();
        try{
            utils.addCloudydata(mapper,"自助服务中心—匹配模型定制",this.getUserPrincipal().getUserCode(),"zzfwzx/glppmx/initGlppmx");
        }catch (Exception e){
    ...
    public void list(GlppmxVO svo, int limit, int offset, String sort, String order) {
        ...
        param.put("svo", svo);
        PaginatedVO vo = delegateMapper.selectPaginated("com.xalt.zzfwzx.dao.GlppmxMapper.selectAll", param, offset, limit);
        includeJson(vo);
    }
    ...
    public void add(GlppmxVO svo) {
        ...
        if (svo.getMxid() == null || svo.getMxid().isEmpty()) {
            delegateMapper.insert("com.xalt.zzfwzx.dao.GlppmxMapper.insertdata", param);
        } else {
            delegateMapper.update("com.xalt.zzfwzx.dao.GlppmxMapper.updatedata", param);
        }
        includeJson("执行成功!");
    }
    ...
    public void deletes(GlppmxVO svo) {
        ...
        param.put("svo", svo);
        DelegateMapper delegateMapper = getMapper();
        delegateMapper.delete("com.xalt.zzfwzx.dao.GlppmxMapper.deletes", param);
        delegateMapper.delete("com.xalt.zzfwzx.dao.GlppmxMapper.deleteschildren", param);
        includeJson("执行成功!");
    }
    ...
    public void mxgzlist(GlppmxVO svo, int limit, int offset) {
        ...
        Map<String, Object> param = Maps.newHashMap();
        param.put("svo", svo);
        PaginatedVO vo = delegateMapper.selectPaginated("com.xalt.zzfwzx.dao.GlppmxMapper.mxgzlist", param, offset, limit);
        includeJson(vo);
    }
    ...
    public void initadd(GlppmxVO svo) {
        ...
        if (svo != null) {
            ...
            Map<String, Object> param = Maps.newHashMap();
            param.put("svo", svo);
            List basicinfo = delegateMapper.selectList("com.xalt.zzfwzx.dao.GlppmxMapper.basicinfo", param);
            //获取所有表
            List alltab = delegateMapper.selectList("com.xalt.zzfwzx.dao.GlppmxMapper.getalltab", param);
//            //源表字段
//            List yblist=delegateMapper.selectList("com.xalt.zzfwzx.dao.GlppmxMapper.ybcolumns",param);
//            //目标表字段
//            List mbblist=delegateMapper.selectList("com.xalt.zzfwzx.dao.GlppmxMapper.mbbcolumns",param);
//            //规则列表
//            List gzlist=delegateMapper.selectList("com.xalt.zzfwzx.dao.GlppmxMapper.gzlist",param);
...
//            results.put("gzlist",gzlist);
        ...
        if (svo != null) {
            ...
            for (Object vo : myulist) {
                ...
                if (kr.getMxgzid() == null || kr.getMxgzid().isEmpty()) {
                    delegateMapper.insert("com.xalt.zzfwzx.dao.GlppmxMapper.insertmxgz", param1);
                } else {
                    delegateMapper.update("com.xalt.zzfwzx.dao.GlppmxMapper.updatemxgz", param1);
                }
    ...
    public void therules() {
        DelegateMapper mapper = getMapper();
        List vo = mapper.selectList("com.xalt.zzfwzx.dao.GlppmxMapper.rulesofthemenu");
        includeJson(vo);
    }
    //源数据表、目标数据表(下拉菜单)
    @NoLoginValidate
    public void tablemenu() {
        DelegateMapper mapper = getMapper();
        List vo = mapper.selectList("com.xalt.zzfwzx.dao.GlppmxMapper.tablemenu");
        includeJson(vo);
    }
    //自定义处理任务(新增)
    @NoLoginValidate
    public void newaddition(String datalist, GlppmxVO svo) {
        ...
        Map<String, Object> param = Maps.newHashMap();
        param.put("svo", svo);
        mapper.update("com.xalt.zzfwzx.dao.GlppmxMapper.newaddition", param);
        //查询任务ID
        List<GlppmxVO> rwidlist = mapper.selectList("com.xalt.zzfwzx.dao.GlppmxMapper.viewRwid");
        param.put("mxid", ((GlppmxVO) rwidlist.get(0)).getMxid());
        param.put("ppmxlist", list);
        //批量插入数据项、指标
        mapper.insert("com.xalt.zzfwzx.dao.GlppmxMapper.addTempdata", param);
        includeJson("新增成功!");
    }
}
