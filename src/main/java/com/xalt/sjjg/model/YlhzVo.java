package com.xalt.sjjg.model;

import com.ts.model.BaseVO;

/**
 * Created by cuiheng on 2018/7/31.
 * 医疗汇总
 */
public class YlhzVo extends BaseVO {

    private String xh;
    private String jfrsbh;
    private String jfrmc;
    private String hsjg;
    private String jfrksswjg;
    private String sbsjly;
    private String sbjbjg;
    private String sbbm;
    private String zsxmdm;
    private String zsxm;
    private String zspmdn;
    private String zspm;
    private String zszmdm;
    private String zszm;
    private String fkssqz;
    private String jfrs;
    private String jfjs;
    private String jfbl;
    private String pzhm;
    private String kpjbr;
    private String kpjbrq;
    private String dzsph;
    private String rkrq;
    private String rkxhjbr;
    private String rkxhjbrq;
    private String sjje;
    private String lxsm;


    public String getXh() {
        return xh;
    }

    public void setXh(String xh) {
        this.xh = xh;
    }

    public String getJfrsbh() {
        return jfrsbh;
    }

    public void setJfrsbh(String jfrsbh) {
        this.jfrsbh = jfrsbh;
    }

    public String getJfrmc() {
        return jfrmc;
    }

    public void setJfrmc(String jfrmc) {
        this.jfrmc = jfrmc;
    }

    public String getHsjg() {
        return hsjg;
    }

    public void setHsjg(String hsjg) {
        this.hsjg = hsjg;
    }

    public String getJfrksswjg() {
        return jfrksswjg;
    }

    public void setJfrksswjg(String jfrksswjg) {
        this.jfrksswjg = jfrksswjg;
    }

    public String getSbsjly() {
        return sbsjly;
    }

    public void setSbsjly(String sbsjly) {
        this.sbsjly = sbsjly;
    }

    public String getSbjbjg() {
        return sbjbjg;
    }

    public void setSbjbjg(String sbjbjg) {
        this.sbjbjg = sbjbjg;
    }

    public String getSbbm() {
        return sbbm;
    }

    public void setSbbm(String sbbm) {
        this.sbbm = sbbm;
    }

    public String getZsxmdm() {
        return zsxmdm;
    }

    public void setZsxmdm(String zsxmdm) {
        this.zsxmdm = zsxmdm;
    }

    public String getZsxm() {
        return zsxm;
    }

    public void setZsxm(String zsxm) {
        this.zsxm = zsxm;
    }

    public String getZspmdn() {
        return zspmdn;
    }

    public void setZspmdn(String zspmdn) {
        this.zspmdn = zspmdn;
    }

    public String getZspm() {
        return zspm;
    }

    public void setZspm(String zspm) {
        this.zspm = zspm;
    }

    public String getZszmdm() {
        return zszmdm;
    }

    public void setZszmdm(String zszmdm) {
        this.zszmdm = zszmdm;
    }

    public String getZszm() {
        return zszm;
    }

    public void setZszm(String zszm) {
        this.zszm = zszm;
    }

    public String getFkssqz() {
        return fkssqz;
    }

    public void setFkssqz(String fkssqz) {
        this.fkssqz = fkssqz;
    }

    public String getJfrs() {
        return jfrs;
    }

    public void setJfrs(String jfrs) {
        this.jfrs = jfrs;
    }

    public String getJfjs() {
        return jfjs;
    }

    public void setJfjs(String jfjs) {
        this.jfjs = jfjs;
    }

    public String getJfbl() {
        return jfbl;
    }

    public void setJfbl(String jfbl) {
        this.jfbl = jfbl;
    }

    public String getPzhm() {
        return pzhm;
    }

    public void setPzhm(String pzhm) {
        this.pzhm = pzhm;
    }

    public String getKpjbr() {
        return kpjbr;
    }

    public void setKpjbr(String kpjbr) {
        this.kpjbr = kpjbr;
    }

    public String getKpjbrq() {
        return kpjbrq;
    }

    public void setKpjbrq(String kpjbrq) {
        this.kpjbrq = kpjbrq;
    }

    public String getDzsph() {
        return dzsph;
    }

    public void setDzsph(String dzsph) {
        this.dzsph = dzsph;
    }

    public String getRkrq() {
        return rkrq;
    }

    public void setRkrq(String rkrq) {
        this.rkrq = rkrq;
    }

    public String getRkxhjbr() {
        return rkxhjbr;
    }

    public void setRkxhjbr(String rkxhjbr) {
        this.rkxhjbr = rkxhjbr;
    }

    public String getRkxhjbrq() {
        return rkxhjbrq;
    }

    public void setRkxhjbrq(String rkxhjbrq) {
        this.rkxhjbrq = rkxhjbrq;
    }

    public String getSjje() {
        return sjje;
    }

    public void setSjje(String sjje) {
        this.sjje = sjje;
    }

    public String getLxsm() {
        return lxsm;
    }

    public void setLxsm(String lxsm) {
        this.lxsm = lxsm;
    }
}
